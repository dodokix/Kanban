    ...
    switch(msg->type) {
        ...
        case CMD_START_AUCTION: {
            ...
            for(int i=0; i < current_auction.expected_count; i++) {
                int peer_port = current_auction.user_list[i];
                if(peer_port <= 0) continue;

                /* Anti-doppioni: la connessione la apre solo chi ha la porta
                   piu' bassa.
                */
                if(my_port < peer_port || get_peer_socket(peer_port) >= 0)
                    send_to_peer(&bid_msg, peer_port);
            }

            /* Alcuni peer potrebbero aver gia' inviato la loro offerta prima
               che noi ricevessimo START_AUCTION: in tal caso calcola subito. */
        ...
        case CMD_PING_USER: {
            ...
            send_to_server(&pong);
            printf("[PONG] inviato pong alla lavagna\n");
            break;
        }
...
void handle_peer_message(Message* msg, int peer_sock) {
    ...
    /* 
        serve a salvare la porta del client 
        in quanto il mittente non manda la propria porta di ascolto quando instaura una nuova connessione 
    */
    update_peer_port(peer_sock, msg->sender_port);

    if(msg->card_id != current_auction.card_id)
        return;

    if(current_auction.received_count >= MAX_USERS){
        printf("[WARN] Ricevuti troppi valori\n");
        return;
    }
    ...
    /* 
        Se la nostra porta e' piu' alta, la connessione l'ha aperta il peer
    */
    if(current_auction.active && my_port > msg->sender_port) {
        Message bid_msg;
        memset(&bid_msg, 0, sizeof(Message));
        bid_msg.type = CMD_CHOOSE_USER;
        bid_msg.sender_port = my_port;
        bid_msg.cost = current_auction.my_cost;
        bid_msg.card_id = current_auction.card_id;
        send_to_peer(&bid_msg, msg->sender_port);
    }

    if(current_auction.active &&
       current_auction.received_count >= current_auction.expected_count) {
        check_auction_result();
    }
}
...
void handle_stdin() {
    ...
    if(strncmp(buffer, "create", strlen("create")) == 0){
        ...
        send_to_server(&msg);
        printf("[CREATE]: inviata card da creare\n");
        return;
    } 
